...
def reformulate_query(question):
    # Send a message to the model asking it to improve the query with domain names
    openai.api_base = "https://api.openai.com/v1/"
    openai.api_key = config['OPENAI_API_KEY']
    response = client.chat.completions.create(
        model="gpt-3.5-turbo-1106",
        messages=[
            {"role": "system", "content": optimal_query},
            {"role": "user", "content": f' User question: {question}'}
        ]
    )
    # Return the content of the model's response
...
if 'reformulated_query' not in st.session_state:
    st.session_state['reformulated_query'] = ''
